package me.yuanhu.core.DataStructure.tree.bplus;
...
public class BplusNode <K extends Comparable<K>, V> {
    ...
    public void insertOrUpdate(K key, V value, BplusTree4<K,V> tree){
        //如果是叶子节点
        if (isLeaf){
            //不需要分裂，直接插入或更新
    ...
    private void copy2Nodes(K key, V value, BplusNode<K,V> left,
                            BplusNode<K,V> right,BplusTree4<K,V> tree) {
        //左右两个节点关键字长度
    ...
    /** 插入节点后中间节点的更新 */
    protected void updateInsert(BplusTree4<K,V> tree){
    ...
    /** 删除节点后中间节点的更新*/
    protected void updateRemove(BplusTree4<K,V> tree) {
    ...
    public V remove(K key, BplusTree4<K,V> tree){
        //如果是叶子节点
        if (isLeaf){
            //如果不包含该关键字，则直接返回
            if (contains(key) == -1){
