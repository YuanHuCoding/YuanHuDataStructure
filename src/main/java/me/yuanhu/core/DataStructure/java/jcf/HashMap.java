...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * 为什么需要使用加载因子，为什么需要扩容呢？因为如果填充比很大，说明利用的空间很多，如果一直不进行扩容的话，链表就会越来越长，
     * 这样查找的效率很低，因为链表的长度很大（当然最新版本使用了红黑树后会改进很多），扩容之后，将原来链表数组的每一个链表分成
     * 奇偶两个子链表分别挂在新链表数组的散列位置，这样就减少了每个链表的长度，增加查找效率.
     *
    ...
    /**
         ...
         x. 扩容 = 对哈希表进行resize操作（即重建内部数据结构），从而哈希表将具有大约两倍的桶数
         y. 扩容阈值 = 容量* 加载因子
     */
    int threshold;
...
//            for (Map.Entry<? extends K, ? extends V> e : m.entrySet()) {
    ...
    final Node<K,V> getNode(int hash, Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            // 通过该函数，依次在数组、红黑树、链表中查找（通过equals（）判断）
            // x. 先在数组中找，若存在，则直接返回
    ...
    /**
        ...
        onlyIfAbsent的作用：当key存在的情况下，不会修改value
     */
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // x. 当前位置的key是否与需插入的key相同
        // y. 判断需插入的数据结构是否为红黑树 or 链表
        if ((p = tab[i = (n - 1) & hash]) == null)
            tab[i] = newNode(hash, key, value, null);
        else {
            Node<K,V> e; K k;
            // x. 判断 table[i]的元素的key是否与 需插入的key一样，若相同则 直接用新value 覆盖 旧value
            // 判断原则：equals（）
            if (p.hash == hash &&
                    ((k = p.key) == key || (key != null && key.equals(k))))
                e = p;
            // y. 继续判断：需插入的数据结构是否为红黑树 or 链表
    ...
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {//超过MAXIMUM_CAPACITY,无法扩容只能改变 阈值
            ...
            // 针对情况2：若未超过最大值，就扩充为原来的2倍
                ...
                newThr = oldThr << 1; // double threshold  // 通过右移扩充2倍
        }
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    else if (e instanceof TreeNode) //下面是分2种情况操作，一种是发生碰撞的节点以树结构进行存储，另一种是以链表结构存储
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);//这里是处理树的情况
                    else { // preserve order  保持顺序// 链表优化重hash的代码块
                        /*
                            下面我们讲解下JDK1.8做了哪些优化。经过观测可以发现，我们使用的是2次幂的扩展(指长度扩为原来2倍)，
                            所以，经过rehash之后，元素的位置要么是在原位置，要么是在原位置再移动2次幂的位置。
                        * */
    ...
    final Node<K,V> removeNode(int hash, Object key, Object value,
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
                (p = tab[index = (n - 1) & hash]) != null) { // 1. 定位桶位置
            ...
            // 3. 删除节点，并修复链表或红黑树
            // 获取到 key 对应的节点 ， 判断是否要进行值匹配treeifyBin
    ...
    public void clear() {
        ...
        if ((tab = table) != null && size > 0) {
            ...
            for (int i = 0; i < tab.length; ++i)
                tab[i] = null;// 很简单把数组中每个位置设置为 null
        }
    }
...
//            SharedSecrets.getJavaOISAccess().checkArray(s, Map.Entry[].class, cap);
    ...
    // Create x regular (non-tree) node
    ...
    /*
        ...
        LinkedHashMap靠重写这3个方法就完成了LRU算法的核心功能。不得不感叹，LinkedHashMap设计之妙。
        在LinkedHashMap的get()方法中，我们每次获取元素的时候，都要调用afterNodeAccess(e)都要将元素移动到尾部。
    ...
    * */
    ...
    /* ------------------------------------------------------------ */
    // DataStructure_Tree bins

    /**
     * Entry for DataStructure_Tree bins. Extends LinkedHashMap.Entry (which in turn
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
         ...
         * hashCodes and non-comparable. We don't require x total
         * order, just x consistent insertion rule to maintain
         * equivalence across rebalancings. Tie-breaking further than
         * necessary simplifies testing x bit.
         */
         ...
         * @return root of tree
            作用：将Treenode链转化成红黑树
            但是我们发现，之前的操作并没有设置红黑树的颜色值，现在得到的只能算是个二叉树。
            在 最后调用树形节点 hd.treeify(tab) 方法进行塑造红黑树。
            可以看到，将二叉树变为红黑树时，需要保证有序。这里有个双重循环，拿树中的所有节点和当前节点的哈希值
            进行对比(如果哈希值相等，就对比键，这里不用完全有序），然后根据比较结果确定在树种的位置。
         */
        ...
        /**
         * Returns x list of non-TreeNodes replacing those linked from
         * this node.
         */
         ...
         * cannot swap the contents of an interior node with x leaf
         * successor that is pinned by "next" pointers that are accessible
         * independently during traversal. So instead we swap the tree
         * linkages. If the current tree appears to have too few nodes,
         * the bin is converted back to x plain bin. (The test triggers
        ...
        /**
         * Splits nodes in x tree bin into lower and upper tree bins,
